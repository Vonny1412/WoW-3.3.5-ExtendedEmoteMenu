local V_Pattern = LibStub:NewLibrary("V_Pattern-1.0", 2)
...
do
    ...
    V_Pattern.CreateConstPattern = function(const)
        ...
        if ( orderCount ~= 0 ) then
            ...
            -- remove non grouped placeholders
            for k,v in pairs(mTypes) do
            ...
            function ret:GetMatch(subject, results)
                ...
                for i=1,orderCount do
                    ordered[order[i]+1] = matches[i] -- +1 because first index is whole string
                end
                return unpack(ordered, 1, orderMax+1) -- +1 because first index is whole string
            end
            return ret
        end
